...
find "$FOLDER" -maxdepth 1 -type f -name "*.docx" | while read -r FILE; do
	...
	#echo $CREATION_DATE
	#The documents won't display in WikiJs if there is any spaces in between the words.
	NEWBASENAME="${BASENAME// /_}"

	#Only if the md file doesn't exist, then it will create a new one, otherwise it will skip it
